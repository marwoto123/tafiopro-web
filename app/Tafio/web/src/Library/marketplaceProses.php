...
trait marketplaceProses
{
    public function marketplaceOrder()
    {
        ...
        DB::transaction(function () {
            ...
            $ambil = DB::table('produks')->select('produks.id', 'hpp', 'stok', 'id_produk', 'harga')->
...
$yy=produkStok::lastStok()->where('cabang_id',$toko)->get();
        foreach ($yy as $row) {
$lastStok[$row->produk_id]=$row->saldo;
        }
...
if($input){
            ...
            if ($batal) {
                ...
                if ($produkBatal) {
                    foreach ($produkBatal as $produk_id => $stokx) {
                        ...
                        $saldo = ($lastStok[$produk_id]??0) + $stokx;
                        $inputBatal[] = array(
                            'produk_id' => $produk_id,
                            'tambah' => $stokx,
                            'saldo' => $saldo,
            ...
            if ($input) {
                ...
                $orderdetil_awal = DB::table('project_details')->where('nota', $nota_awal)->where('company_id', session('company'))->first()->id;
...
SET project_id = (
    ...
    WHERE projects.nota=project_details.nota
		...
		limit 1
) where id>=" . $orderdetil_awal . " and company_id=" . session('company') . " ");
    ...
    public function marketplaceStok()
    {
        ...
        $ambil = DB::table('produks')->select('produks.id',  'stok', 'id_produk', 'harga')->
            where('produks.status', 1)
            ->where('produks.company_id', session('company'))
            ->join('produk_models', 'produks.produk_model_id', '=', 'produk_models.id')
            ->get();
...
$lastStok[$row->produk_id]=$row->saldo;
        ...
        while (($baris = fgetcsv($file_excel, 1000, ",")) !== false) {
            ...
            $sku = !empty($sku_induk) ? $sku_induk : $sku_anak;
